...
router.post("/", async (req, res) => {
  const { type, title = null, message } = req.body;
  const createdAt = new Date().toISOString();

  const insertQuery = `
    INSERT INTO notifications (type, title, message, createdAt)
    VALUES (?, ?, ?, ?)
  `;

  db.run(insertQuery, [type, title, message, createdAt], async function (err) {
    if (err) {
